...
import (
	...
	"github.com/crossplane/provider-dex/apis"
	dex "github.com/crossplane/provider-dex/internal/controller"
	"github.com/crossplane/provider-dex/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "Dex support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-dex"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-dex",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Dex APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-dex:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(dex.SetupGated(mgr, o), "Cannot setup Dex controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(dex.Setup(mgr, o), "Cannot setup Dex controllers")
	}
